...
export default async function PaymentsPage() {
  const { payments, overduePayments, summary, tenants, leases, meta } = await getLandlordPaymentsVm();
  const collected = formatMoneyBreakdown(
    payments
      .filter((payment) => payment.status === "paid")
      .map((payment) => ({ amount: payment.amount, currency: payment.currency })),
  );
  const outstanding = formatMoneyBreakdown(
    payments
      .filter((payment) => payment.status === "pending")
      .map((payment) => ({ amount: payment.amount, currency: payment.currency })),
  );
  const overdue = formatMoneyBreakdown(
    overduePayments
      .map((payment) => ({ amount: payment.amount, currency: payment.currency })),
  );
              ...
              {payments.map((payment) => (
                <tr key={payment.id} className="border-t border-[var(--secondary)]">
                  <td className="px-6 py-5">
                    ...
                    <p className="text-xs text-secondary-2">
                      {payment.unitLabel ?? payment.unitId ?? "Unité non liée"}
                    </p>
                  </td>
