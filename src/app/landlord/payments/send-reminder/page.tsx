    ...
    <LandlordPageFrame currentPath="/landlord/payments">
      ...
      <section className="grid gap-8 lg:grid-cols-2">
        <SurfaceCard className="p-6">
          <h2 className="text-xl font-bold text-foreground">Relance de paiement</h2>
          <div className="mt-5 rounded-xl bg-[var(--secondary-4)] p-4">
            <p className="font-semibold text-foreground">{relatedTenant?.fullName ?? "Locataire indisponible"}</p>
            <p className="text-xs text-secondary-2">{overduePayment?.unitId || "Unité indisponible"} • Contexte du paiement</p>
          </div>
          <SendPaymentReminderForm payments={payments} tenants={tenants} />
        </SurfaceCard>

        <SurfaceCard className="rounded-[28px] bg-[linear-gradient(145deg,var(--primary-1),var(--primary-2))] p-8 text-white">
